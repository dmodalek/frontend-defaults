...
export function validateConfigPresetJSON<T extends object>({ name, config, defaults, fields }: ValidateConfigPresetJSONOpts<T>): ValidationResult[] {
    if (!config) {
        return [{
    ...
    const configurationMissmatches = fields.map((field, pos) => {
        return {
            field,
            used: config[field],
